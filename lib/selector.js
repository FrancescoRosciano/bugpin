...
/** Utility-CSS class names (Tailwind and friends) escape into unreadable noise. */
const MAX_CLASS_LENGTH = 24;
const MAX_CLASSES_PER_SEGMENT = 2;

export function classChain(node) {
  const stable = getClassTokens(node)
    .filter((token) => isStableId(token) && token.length <= MAX_CLASS_LENGTH)
...
/**
 * True when `selector` resolves to exactly `el`. Returns null when the document
 * cannot answer — the fake DOMs in the tests have no querySelectorAll, and a
 * caller must then fall back to building a full structural path.
 */
function resolvesUniquely(el, selector) {
  const doc = el.ownerDocument;
  if (!selector || !doc || typeof doc.querySelectorAll !== 'function') return null;
  try {
    const found = doc.querySelectorAll(selector);
    return found.length === 1 && found[0] === el;
  } catch {
    return false; // an unescapable class name, say
  }
}
...
const MAX_ANCESTOR_STEPS = 6;

/**
 * Grows a selector from the element outwards, stopping at the first one that
 * resolves to exactly this element. Returns null when the document cannot
 * answer uniqueness questions, leaving the caller its structural fallback.
 */
function shortestUniqueSelector(el) {
  for (const candidate of ownSegmentCandidates(el)) {
    const unique = resolvesUniquely(el, candidate);
    if (unique === null) return null; // no querySelectorAll — caller falls back
    if (unique) return candidate;
  }
  let suffix = segmentFor(el);
  let node = el.parentElement;
  for (let step = 0; node && node.tagName && step < MAX_ANCESTOR_STEPS; step++) {
    if (hasUiMarker(node)) {
      node = node.parentElement;
      continue;
    }
    const anchor = stableSelectorFor(node);
    const prefix = anchor || segmentFor(node);
    suffix = `${prefix} > ${suffix}`;
    if (resolvesUniquely(el, suffix)) return suffix;
    if (anchor) break; // an anchored path that is still ambiguous will not improve
    node = node.parentElement;
  }
  return null;
}
...
/**
 * Candidates for the element's own segment, shortest first. Real pages carry
 * deep utility-class trees, so a selector is grown only until it is unique
 * rather than always walked up to <body>.
 */
function ownSegmentCandidates(el) {
  const tag = el.tagName.toLowerCase();
  const classes = classChain(el);
  return [
    stableSelectorFor(el),
    classes ? `${tag}${classes}` : null,
    `${tag}:nth-of-type(${nthOfTypeIndex(el)})`,
    segmentFor(el),
  ].filter(Boolean);
}
...
export function cssSelectorFor(el) {
  ...
  const shortest = shortestUniqueSelector(el);

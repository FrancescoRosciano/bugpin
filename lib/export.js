...
/**
 * Two different sessions in the same minute whose first note reads the same
 * produce the same folder name — and since re-export overwrites by design
 * (PROTOCOL §4), the second one would silently destroy the first one's
 * evidence. Re-exporting the SAME session must still land on the same folder,
 * so disambiguation is keyed by session id, not by mere existence.
 *
 * @param {string} base folderName(session)
 * @param {Record<string,string>} claims folder -> session id of whoever wrote it
 * @param {string} sessionId the session exporting now
 */
export function claimFolder(base, claims, sessionId) {
  const taken = claims || {};
  if (!taken[base] || taken[base] === sessionId) return base;
  for (let n = 2; n < 1000; n++) {
    const candidate = `${base}-${n}`;
    if (!taken[candidate] || taken[candidate] === sessionId) return candidate;
  }
  throw new Error(`cannot find a free export folder for ${base}`);
}
...
/** The placeholder lib/redact.js leaves behind — never a value a spec can replay. */
const REDACTED = '«redacted»';

function playwrightFillLines(selector, value) {
  if (value === REDACTED) {
    return [
      `  // TODO: the real value was redacted — supply it via the environment.`,
      `  await page.locator('${selector}').fill(process.env.BUGPIN_SECRET ?? '');`,
    ];
  }
  return [`  await page.locator('${selector}').fill('${escapeJsString(value)}');`];
}
...
function playwrightStepLines(evt) {
  ...
  switch (evt.action) {
    ...
    case 'input':
    case 'change':
      return playwrightFillLines(selector, evt.value ?? '');
    case 'submit':

...
/**
 * Chrome renames a download whose filename extension disagrees with the URL's
 * MIME type — `data:text/plain` turned report.md into report.txt, e2e.spec.ts
 * into e2e.spec.txt and session.json into session.txt. Each extension is served
 * with a type Chrome maps back to that same extension; anything unrecognised
 * falls back to octet-stream, which Chrome leaves alone.
 */
...
export function textMimeFor(path) {
  const ext = String(path).split('.').pop()?.toLowerCase();
  return TEXT_MIME_BY_EXTENSION[ext] || 'application/octet-stream';
}

function textDataUrl(text, path) {
  return `data:${textMimeFor(path)};charset=utf-8,${encodeURIComponent(text)}`;
}
...
function textEntries(files) {
  if (Array.isArray(files)) {
    return files.map((f) => ({ path: f.path, url: textDataUrl(f.content, f.path) }));
  }
  return Object.entries(files || {}).map(([path, text]) => ({ path, url: textDataUrl(text, path) }));
}
